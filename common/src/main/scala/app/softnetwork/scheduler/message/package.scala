...
package object message {
  ...
  /** To schedule a message to be sent repeatedly to the self actor as long as the cron expression
    * is satisfied ( as long as next() will return Some) key - a unique identifier for the
    * underlying schedule cronExpression - a simplified subset of the time expression part of the V7
  ...
  /** To schedule a message to be sent either once after the given `delay` or repeatedly with a
    ...
    *
    * @tparam C
    *   - the type of Command to schedule
    */
  case class ScheduleCommand[C <: Command](
    key: Any,
    command: C,
  ...
  object ScheduleCommand {
    ...
    implicit def cronTabCommandToSchedule[C <: Command](
  ...
  trait SchedulerCommand extends EntityCommand {
    override lazy val id: String = SchedulerSettings.SchedulerConfig.id.getOrElse(ALL_KEY)
  }

  private[scheduler] case class ResetCronTabsAndSchedules(resetScheduler: Boolean = false)
      extends SchedulerCommand

  private[scheduler] case object ResetScheduler extends SchedulerCommand

  case class TriggerSchedule(persistenceId: String, entityId: String, key: String)
      extends SchedulerCommand
      with SchedulerItem

  case class TriggerCronTab(persistenceId: String, entityId: String, key: String)
      extends SchedulerCommand
      with SchedulerItem
  ...
  case object ScheduleNotTriggered extends SchedulerErrorMessage("ScheduleNotTriggered")
